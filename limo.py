...
KEY_WORDS = ["wait", "verify", "check", "realize", "note that", "recall that"]
...
def calculate_token(df):
    ...
    for _, row in df.iterrows():
        ...
        # Calculate lines (split by double newlines)
        lines = len(text.split('\n\n'))
        total_line_count += lines
        
        # Calculate tokens per line for this sample
        tokens_per_line = tokens / lines if lines > 0 else 0
        total_tokens_per_line += tokens_per_line
        
        total_problem_count += 1
    
    avg_tokens = total_token_count/total_problem_count
    avg_lines = total_line_count/total_problem_count
    avg_tokens_per_line = total_tokens_per_line/total_problem_count
    
    st.subheader(f"Average Token: {avg_tokens:.2f}")
    st.subheader(f"Average Lines: {avg_lines:.2f}")
    st.subheader(f"Average Tokens per Line: {avg_tokens_per_line:.2f}")
    

def statistics_key_words(df):
    key_word_count = {key_word: 0 for key_word in KEY_WORDS}
    ...
    for _, row in df.iterrows():
        ...
        # Count all occurrences of each keyword in the text
        for key_word in KEY_WORDS:
            # Use count() to get all occurrences in the text
            key_word_count[key_word] += text.count(key_word)
    
    # Calculate average occurrences per sample
    avg_occurrences = {
        key: count/total_samples 
        for key, count in key_word_count.items()
    }
    
    fig, ax = plt.subplots(figsize=(10, 6))
    bars = ax.bar(avg_occurrences.keys(), avg_occurrences.values())
    ...
    ax.set_ylabel('Average Occurrences per Sample')
    ax.set_title('Average Keyword Occurrences per Sample')
...
def visualize_limo():
    ...
    if file_type == "Training Data":
        ...
        st.write("Quality: deepseek-r1 > deepseek-r1-distill-qwen-32b > our_model > qwen2.5-32b-instruct > official-solution")
        
        folder_path = './data/limo/training_data'
    ...
    def calculate_sample_stats(text):
        tokens = len(enc.encode(text))
        lines = len(text.split('\n\n'))
        tokens_per_line = tokens / lines if lines > 0 else 0
        return tokens, lines, tokens_per_line


    if file_type == "Training Data":
        def show_cot(row, data_name):
            st.subheader(f"CoT of {data_name}")
            tokens, lines, tokens_per_line = calculate_sample_stats(row['solution'])
            st.write(
                f"Token: {tokens} | "
                f"Lines: {lines} | "
                f"Tokens per Line: {tokens_per_line:.2f}"
            )
